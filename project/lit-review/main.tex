...
namespaces via a capbility-based implementation \cite{watson2010_capsicum}. In
...
Much like seccomp-bpf and pledege, however, Capsicum is \textit{not} application
...
emerged \cite{docker, snap, flatpak}, allowing users to download applications as
packages which are then confined using a combination of lower level techniques
exposed by the operating system. Typically a package maintainer or application
author would write a high-level package manifest declaring the privileges required
by the application. This package manifest would then be translated into policy
for the underlying process confinement mechanisms. This architecture is depicted
in \Cref{fig:containerized}.
