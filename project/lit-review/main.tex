...
\Cref{fig:syscall_interposition} presents an overview of the usability, security,
and granularity trade-offs in each framework.

\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.8\linewidth]{figs/systemcall-interposition.pdf}
    \caption{Usability, security, and granularity trade-offs of pledge,
...
\paragraph*{Docker, Snap, and Flatpak}
...
As a motivating example, consider the package manifest for Snap's Apache
\texttt{httpd} package. \todo{present package manifest...}

\todo{Firejail, Bubblewrap}
